...
_C.MODEL.SWIN.OUT_INDICES = (0, 1, 2, 3)
...
def update_config(config, args):
    ...
    if _check_args('dataset_file'):
        config.DATASET_FILE = args.dataset_file
    if _check_args('num_workers'):
        config.NUM_WORKS = args.num_workers  
    if _check_args('data-root'):
        config.DATA_ROOT = args.data_root  
    if _check_args('point_loss_coef'):
        config.POINT_LOSS_COEF = args.point_loss_coef  
    if _check_args('eos_coef'):
        config.EOS_COEF = args.eos_coef  
    if _check_args('set_cost_class'):
        config.SET_COST_CLASS = args.set_cost_class  
    if _check_args('set_cost_point'):
        config.SET_COST_POINT = args.set_cost_point 
